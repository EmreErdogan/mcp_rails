...
module McpRails
  ...
  class McpController < ApplicationController
    # Always skip CSRF protection for MCP endpoints since they use JSON-RPC
    ...
    def skip_authentication?
      # Skip authentication for internal requests from stdio server
      internal_request? || McpRails.configuration.auth_strategy == :none
    end

    def internal_request?
      request.headers['X-MCP-Internal'] == 'true'
    end
  end
end